...
use meow::solana::svm_manager::{init_solana_manager, SOLANA_MANAGER,SolanaManager};
use anyhow::anyhow;
...
async fn main() {
    ...
    match app.command {
        Command::Bridge { mainnet,amount ,to_chain,to } => {
      


            
            let domain = DestinationDomain::from_str(&to_chain)
            .ok_or_else(|| anyhow!("Invalid chain: {}", to_chain)).unwrap();
        ...
        let  deposit_for_burn_sig = meow::solana::programs::call_deposit_for_burn(fixed_domain,&to,amount).await.unwrap();
        let attestation_data = get_messages(&deposit_for_burn_sig).await.unwrap();
             evm_claim(&attestation_data.message ,&attestation_data.attestation ,fixed_domain,mainnet).await.unwrap();
