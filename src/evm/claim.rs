...
pub async fn evm_claim(message_st :&str , attestation: &str ,domain: u32,mainnet:bool) -> web3::contract::Result<H256> {
   ...
   let contract = evm.message_transmitter_contract;
    let message: Vec<u8> = hex::decode(message_st.trim_start_matches("0x"))
    .expect("Invalid hex string");

let attestation: Vec<u8> = hex::decode(attestation.trim_start_matches("0x"))
.expect("Invalid attestation hex string");
...
let tx_hash :H256 = web3.eth().send_raw_transaction(signed_tx.raw_transaction).await?;
