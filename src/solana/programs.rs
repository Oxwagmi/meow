...
use super::svm_manager::{SolanaManager,SOLANA_MANAGER};
...
pub async fn call_deposit_for_burn(destination_domain: u32 ,mint_recipient_address :&str ,amount_in_usdc:u64) -> Result<Signature> {
    let manager:&SolanaManager = SOLANA_MANAGER.get().unwrap();
    let payer = manager.payer();
    let client = manager.client();
    let usdc_address = Pubkey::from_str("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU")?;
    let user_token_account = Pubkey::from_str("9UP1hkKwpXQw6KRwDxa41VUYebtWR6eXnE9ddi7tkzf8")?;
    ...
    println!("  message_transmitter_account: {}", pdas.message_transmitter_account);
    println!("  token_messenger_account: {}", pdas.token_messenger_account);
    println!("  token_minter_account: {}", pdas.token_minter_account);
    println!("  local_token: {}", pdas.local_token);
    println!("  remote_token_messenger_key: {}", pdas.remote_token_messenger_key);
    ...
    println!("  message_sent_event_data: {}", message_sent_event_account_keypair.pubkey());
